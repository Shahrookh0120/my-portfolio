...
export const TypewriterEffectSmooth = ({ words, className, cursorClassName }) => {
  ...
  const renderWords = () => {
    return (
      <div>
        {wordsArray.map((word, idx) => (
          <div key={`word-${idx}`} className="inline-block ms-4">
            {word.text.map((char, index) => (
              <span
                key={`char-${index}`}
                className={cn(`dark:text-white text-black xl:text-[64px] font-extrabold xl:tracking-[.5rem] sm:tracking-[0.3rem]`, word.className)}>
                {char}
              </span>
            ))}
          </div>
